...
public class ProjectService{
    ...
    public static Set<ProjectsDTO> getProjectsByTitle(String projectName) {
        ...
        try {
            Document document = Jsoup.connect(consts.url).get();
            Elements elements = document.getElementsByTag("a");
                ...
                for (Element ads : elements) {
                    if (ads.attr("title").equals(projectName)) {
                        ...
                        projectsDTO.setHref(consts.url + "/tree/main/" + projectName);

                        Document connectionMidProject = Jsoup.connect(consts.url + "/tree/main/" + projectName).get();
                        ...
                        for(Element adsNext : elementsMidProject){
                            if (adsNext.attr("title").equals("readme.md")){
                                Document connectionReadmeMd = Jsoup.connect(consts.url + "/blob/main/" + projectName + "/readme.md").get();
        ...
        try{
            ...
            for (Element ads : elements) {
                if (ads.attr("title").equals("") || ads.attr("title").equals("GitHub")){
                    ...
                    projectsDTO.setTitle(ads.attr("title"));
                    projectsDTO.setHref(consts.url + "/");
